...
import com.xwc.esbatis.anno.condition.Condition;
...
public class AnalysisAssistant {
    ...
    private String templateIf = "<if test='%s != null' > %s </if>";
    ...
    public QueryMate parseQueryEntity(Method method) {
        ...
        for (Class<?> par : parameterTypes) {
            ...
            for (Field f : fields) {
                Ignore ignore = AnnotationUtils.findAnnotation(f, Ignore.class);
                if (ignore != null) continue;
                Condition annotation = AnnotationUtils.findAnnotation(f, Condition.class);
                query.addFilter(analysisFilterColum(annotation, f, index));
                ++index;
            }
        }
        return query;
    ...
    public QueryMate parseQueryMethod(Method method) {
        ...
        for (int i = 0; i < paramCount; ++i) {
            FilterColumMate paramFilter = getParamFilter(parameterAnnotations[i], paramNames.get(i), i);
            query.addFilter(paramFilter);
        }
        return query;
    }

    private FilterColumMate getParamFilter(Annotation[] annotations, String paraName, int index) {
        Condition condition = null;
        Annotation anno = null;
        for (Annotation annotation : annotations) {
            condition = AnnotationUtils.findAnnotation(annotation.getClass(), Condition.class);
            if (condition != null) {
                anno = annotation;
            }
        }
        ...
        if (anno == null) {
            ...
            filter.setColunm(underscoreName(paraName));
            filter.setField(paraName);
        } else {
            try {
                Object colum = anno.getClass().getMethod("colum").invoke(anno);
                Object annoIndex = anno.getClass().getMethod("index").invoke(anno);
                filter.setColunm(colum.toString().isEmpty() ? underscoreName(paraName) : colum.toString());
                filter.setField(paraName);
                filter.setIndex(((Integer) annoIndex) == -1 ? index : (Integer) annoIndex);
                filter.setConditionEnum(condition.conditionType());
    ...
    public FilterColumMate analysisFilterColum(Condition condition, Field field, int index) {
        FilterColumMate mate = new FilterColumMate();
        mate.setField(field.getName());
        if (condition == null) {
            mate.setConditionEnum(ConditionEnum.EQUEL);
            mate.setColunm(underscoreName(mate.getField()));
            mate.setIndex(index);
        } else {
            mate.setConditionEnum(condition.conditionType());
            mate.setColunm(underscoreName(field.getName()));
            mate.setIndex(index);
        }
        return mate;
    }
    ...
    private StringBuilder buildOrder(FilterColumMate order) {
        ...
        sb.append(String.format(templateIf, order.getField(), String.format(order.getConditionEnum().getSymbol(), order.getBatisColum())));
        return sb;
    }
    ...
    private StringBuilder buildGroup(FilterColumMate group) {
        ...
        sb.append(String.format(templateIf, group.getField(), String.format(group.getConditionEnum().getSymbol(), group.getBatisColum())));
        return sb;
    }
