package cn.onetozero.easybatis.mysql.snippet;
...
public class NotLikeConditional implements SingleConditionalSnippet {
    ...
    public NotLikeConditional(AbstractBatisSourceGenerator sourceGenerator) {
    ...
    public String snippet(BatisColumnAttribute columnAttribute) {
        ...
        NotLike like = columnAttribute.findAnnotation(NotLike.class);
        String conditionSql = "AND " + sqlPlaceholder.holder(columnAttribute.useColumn(like))
                + " NOT LIKE CONCAT('%'," + batisPlaceholder.holder(columnAttribute) + ",'%') ";
