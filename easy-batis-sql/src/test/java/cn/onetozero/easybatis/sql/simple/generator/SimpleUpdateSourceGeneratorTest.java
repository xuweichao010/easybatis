...
public class SimpleUpdateSourceGeneratorTest {
    ...
    public void simpleUpdateBatch() {
        ...
        Method method = Reflection.chooseMethod(interfaceClass, methodName);
        OperateMethodMeta operateMethodMeta = easyBatisConfiguration.getEasyConfiguration().getOperateMethodAssistant().getOperateMethodMeta(interfaceClass, method);
        String expected = "<script>  <foreach item='item' index='index' collection='collection' separator=';' > UPDATE t_user <set> `org_code`=#{item.orgCode}, `org_name`=#{item.orgName}, `name`=#{item.name}, `data_type`=#{item.dataType}, `age`=#{item.age}, `job`=#{item.job}, `create_time`=#{item.createTime}, `create_id`=#{item.createId}, `create_name`=#{item.createName}, `update_time`=#{item.updateTime}, `update_id`=#{item.updateId}, `update_name`=#{item.updateName}, `valid`=#{item.valid}, </set> WHERE `id` = #{item.id} </foreach> </script>";
        Assert.assertEquals(expected, sourceGenerator.update(operateMethodMeta));
    }
